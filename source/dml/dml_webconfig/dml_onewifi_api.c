...
uint8_t get_apmld_index_from_mld_group(mld_group_t *mld_group)
{
    ...
    uint8_t index = (uint8_t)((uintptr_t)mld_group - (uintptr_t)&dml->apmld_map.mld_groups[0]) / sizeof(mld_group_t);
    
    if (index >= dml->apmld_map.mld_group_count) {
        wifi_util_error_print(WIFI_DMCLI, "%s:%d Invalid mld_group pointer, index:%u\n", __func__, __LINE__, index);
        return 0xFF;
    }
...
void update_dml_stamld_list(uint8_t apmld_index)
{
    ...
    mld_group_t *mld_group = &apmld_map->mld_groups[apmld_index];
    hash_map_t **stamld_map = &apmld_map->stamld[apmld_index];
        ...
        while (assoc_dev_temp != NULL) {
            if (assoc_dev_temp->dev_stats.cli_MLDEnable && assoc_dev_temp->dev_stats.cli_Active == true) {
                ...
                if (stamld_entry == NULL) {
                    ...
                    if (stamld_entry == NULL) {
                        wifi_util_error_print(WIFI_DMCLI, "%s:%d Failed to allocate STAMLD entry for index %d\n", 
                            __func__, __LINE__, i);
                        continue;
                    }
                    ...
                    stamld_entry->affiliated_sta[stamld_entry->affiliated_sta_count] = assoc_dev_temp; // TODO Do copy of data here to avoid pointer issues / or no ???
                    ...
                    stamld_entry->affiliated_sta[stamld_entry->affiliated_sta_count] = assoc_dev_temp; // TODO Do copy of data here to avoid pointer issues
                    stamld_entry->affiliated_sta_count++;
                    wifi_util_dbg_print(WIFI_DMCLI, "%s:%d Added affiliated sta for STAMLD[%s], link count: %d\n",
                        __func__, __LINE__, stamld_addr, stamld_entry->affiliated_sta_count);
                }
            }
